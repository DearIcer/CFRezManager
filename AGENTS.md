...
`RezArchiveReader` → `ExplorerItem` tree → thumbnails via `Decoders/*` cached by `ThumbnailDiskCache` → double-click opens `Preview/*` windows; export uses `RezArchiveReader.ExtractFile` + `Decoders/Images/DecodedImageExporter`; repack uses `RezArchiveWriter`.
