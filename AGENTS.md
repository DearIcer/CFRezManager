...
- `Decoders/` — pure format decoders, grouped by kind:
  ...
  - `LithTech/` LTC native decoder, SPR sprites, world DAT; `LithTech/Models/` model decoding (`LtbModelParser` is the structural LTB parser — full header/pieces/skeleton/animation layout, preferred over the heuristic offset scanner in `LithTechModelDecoder` with silent fallback), part grouping, scene building, texture resolution (CFG index + DAT reference index, both persisted across runs by `ModelTextureIndexDiskCache` under `ModelTextureIndexCache/v1/` next to the executable, keyed by a SHA-256 fingerprint of the contributing files' path/size/mtime), OBJ/MTL export, FBX 7.4 binary export (`LithTechFbxExporter` writes geometry/normals/UVs/skeleton/skinning/animation stacks and embeds resolved textures as PNG Video/Texture nodes through the generic `FbxBinaryWriter`; X is mirrored with reversed winding because the source data is left-handed; normals come from the LTB normal stream with an angle-weighted smooth fallback; texture lookup reuses `LithTechObjExporter.ResolveMeshTexture`), thumbnail rendering.
...
- `Preview/` — standalone preview windows: `Audio/` (player with track list, spectrum, FMOD bank progressive loading), `Image/`, `Model/` (Unity viewer embedding), `Text/`.
  - `Model/` hosts the model preview in an embedded Unity player process instead of WPF 3D: `UnityPreviewExporter` exports the decoded `LithTechModelDocument` to a temp OBJ/MTL/PNG set (via `LithTechObjExporter`) under `%TEMP%\CFRezManager\UnityPreview\<guid>\`, `UnityViewerHost` (`HwndHost`) launches the Unity exe with `-parentHWND` and forwards resize/focus, `UnityViewerLocator` resolves the exe. When the document has a skeleton and animations, `UnityPreviewExporter` additionally writes a `preview.cfan` binary animation package (via `Decoders/LithTech/Models/LithTechAnimPackageExporter` — skeleton bind TRS + bindposes, skinned meshes, per-track keyframes, all in source coordinates) and passes it as `--cfrez-anim <path>`; the preview window's animation-track ComboBox switches tracks by writing `anim-command.txt` into the temp directory (commands: `bind`, `track <i>`, `play`, `pause`, `seek <seconds>`), which the viewer polls every 0.1 s; the viewer writes playback state back to `anim-status.txt` (`track <i> <playing 0|1> <time> <length>` or `bind -1 0 0 0`) for the window's play/pause button and seekable progress bar. The Unity project lives outside this repo (currently `E:\UnityProject\RezView`, Unity 2022.3 + URP); its runtime scripts parse `--cfrez-model <path>` (OBJ + free-fly camera) and `--cfrez-anim <path>` (runtime `SkinnedMeshRenderer` + legacy `AnimationClip` playback via `RuntimeAnimPackageLoader`/`AnimCommandWatcher`). Build output goes to `tools\UnityModelViewer\CFRezModelViewer.exe` (rebuild headlessly with `Unity.exe -batchmode -quit -projectPath E:\UnityProject\RezView -executeMethod ViewerBuildScript.BuildViewer`).
...
- The app writes only to user-chosen export paths, `%LocalAppData%\CFRezManager\settings.json`, `ThumbnailCache\` / `RezIndexCache\v1\` / `ModelTextureIndexCache\v1\` folders next to the executable, and the `%TEMP%\CFRezManager\UnityPreview\` temp folder (per-preview OBJ data, deleted on window close and swept on startup); keep new side effects within those locations.
